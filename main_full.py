...
# Сервоприводы
GRIP_SERVO_PIN = 13      # клешня
BUCKET_SERVO_PIN = 33    # ковш
...
def get_exact_text(chunks):
    """
    Расшифровывает NDEF текстовую запись из списка байт-блоков.
    Взято из main.py (пример преподавателя).
    """
    full_data = bytearray()
    for c in chunks:
        full_data.extend(c)
...
def read_rfid_text():
    """Читает RFID-метку и возвращает текстовую строку или None."""
...
def grab_and_read_rfid():
    """Закрывает клешню, читает RFID и отправляет результат."""
    servo_write(grip_pwm, GRIP_CLOSED_ANGLE)
    time.sleep_ms(300)   # пауза на срабатывание серво
